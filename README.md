...
| ------------------- | --------- | ----------- | ---------------- |
| `showDialogBox` | `npc_speaking`, `dialog_title`, `effect` | Fa apparire sopra a <span title="npcs.lua">`npc_speaking`</span> il dialogo `dialog_title` (⇌ `dialogs.lua`) con l'effetto `effect` applicato (⇌ `dialog_effects.lua`) | - |
