...
| ------------------- | --------- | ----------- | ---------------- |
| `showDialogBox` | `npc_speaking`, `dialog_title`, `effect` | Fa apparire sopra a `npc_speaking`<sup>⇌ <code>npcs.lua</code></sup> il dialogo `dialog_title` (⇌ `dialogs.lua`) con l'effetto `effect` applicato (⇌ `dialog_effects.lua`) | - |
